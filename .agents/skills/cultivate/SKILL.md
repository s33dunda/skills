...
- For larger multi-surface repos, a layered set of short nested `AGENTS.md` files in the highest-leverage subtrees. Do this when the repo has distinct implementation, tests, docs/examples, apps, packages, services, or generated-code areas where local guidance can prevent broad, generic patches.
...
- **Mandatory deeper-context links once docs exist.** If cultivate creates or finds root `ARCHITECTURE.md`, `docs/PLANS.md`, `docs/QUALITY.md`, `docs/RELIABILITY.md`, `docs/SECURITY.md`, `docs/design-docs/index.md`, `docs/product-specs/index.md`, `docs/exec-plans/tech-debt-tracker.md`, or `docs/references/`, the root `AGENTS.md` should point at them from a dedicated `## Deeper Context` section. A short AGENTS file that mentions only README plus one or two docs while orphaning the rest of the knowledge store is a weak application of cultivate.
- **Required section shape.** Use exact or near-exact headings for `Orientation`, `Useful Commands` or `Commands`, `Repository Map`, `Deeper Context`, one or more task-scoped work sections (`When Writing Code`, `When Reviewing`, etc.), `Definition of Done`, `When Blocked`, and `Never`. Small repos can keep each section brief, but should not silently omit the map sections once the harness files exist.
...
- **Layered guidance for large repos.** A single root `AGENTS.md` is usually too generic for large repos with separate implementation, test, docs/example, service, package, or generated-code surfaces. Add short nested `AGENTS.md` files to the two to four highest-leverage subtrees. Each local file should name its scope, local working norms, and local validation pairing. Keep local files brief: they extend the root map; they are not subsystem manuals.
...
### Layered Harness Protocol

Use the layered protocol when the audit finds multiple agent-relevant subtrees with different local norms. Common examples: `src/` plus `tests/`, a top-level Python package plus `tests/`, `docs_src/` or `examples/`, `apps/`, `packages/`, `services/`, or generated-code directories.

1. Keep root `AGENTS.md` as the project map: commands, repo map, deeper context, Definition of Done, escalation, and Never list.
2. Add nested `AGENTS.md` only where local guidance will change behavior. Prefer the implementation owner, test owner, and docs/example owner over many shallow files.
3. In each nested file, include:
   - `## Scope`: what this subtree owns.
   - `## Working Norms`: two to five local rules that prevent common broad or misplaced edits.
   - `## Validation` or `## Validation Pairing`: the nearest test, docs, generated-artifact, or command expectation for changes in this subtree.
4. Make nested guidance concrete enough to reduce churn. Point agents toward existing local patterns, nearby tests, and owned flows; discourage introducing new helpers, state, fixtures, or docs surfaces unless the task needs them.
5. Do not add nested files to tiny flat repos just to satisfy a shape. If the repo has no clear local surfaces, record that the root map is enough.

For a study treatment or reusable harness example, a large repo with only root guidance is incomplete unless the audit explains why nested guidance would not help.

### Final AGENTS Self-Check

Before finishing an applied cultivate slice, read the root `AGENTS.md` you just wrote and compare it against `references/templates.md`.

- If the cultivated docs exist but `AGENTS.md` does not link to them, the harness is incomplete.
- If `AGENTS.md` has commands and a Definition of Done but no `Deeper Context`, treat that as a bug, not a stylistic difference.
- If the repo has distinct implementation, test, docs/example, app, package, or service subtrees but no nested `AGENTS.md`, treat that as a treatment-quality gap unless you can explain why the repo is effectively flat.
- Run the bundled audit before handoff when practical. A cultivate slice that only looks good to the writer but fails the local audit is not ready to be a study treatment or a reusable harness example.
