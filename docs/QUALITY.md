...
2. `version` is bumped and `CHANGELOG.md` has a matching entry on every behavioral `SKILL.md` change.
3. Skill helper scripts in `skills/*/scripts/` make no network calls — they are deterministic and run offline.
4. `docs/exec-plans/` holds active and completed ExecPlans; `docs/exec-plans/tech-debt-tracker.md` tracks long-lived cleanup items.
