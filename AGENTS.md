...
- Sync repo-local agent skills: `uv run python scripts/sync_agents_skills.py`
...
- Optional host install through the skills CLI: `npx -y skills@latest add s33dunda/skills/skills/<skill-name>`

The repo is stdlib-only Python; `uv.lock` + `pyproject.toml` declare the runner so agents never fall back to a global interpreter. Bare `python3` is acceptable only when `uv` is unavailable. Repo validation checks `skills/` and the committed `.agents/skills/` mirror only; it does not inspect `~/.codex/skills`, `.claude`, or any other user-local host install.

## Repository Map

- `skills/`: installable skill directories (`plot`, `cultivate`). Each carries `SKILL.md`, `references/`, `scripts/`, `evals/`, `CHANGELOG.md`.
- `.agents/skills/`: committed mirror of `skills/` for agent clients that scan `.agents/` directly. Generated by `scripts/sync_agents_skills.py`; do not edit by hand.
...
- `scripts/sync_agents_skills.py`: maintenance helper that refreshes `.agents/skills/` from canonical `skills/` and removes stale mirrored skills.
...
- After editing anything under `skills/<name>/`, run `uv run python scripts/sync_agents_skills.py` before validation so the committed `.agents/skills/` mirror stays in lockstep.
...
4. If a canonical skill changed: `.agents/skills/<name>/` has been regenerated by `uv run python scripts/sync_agents_skills.py`.
5. If the cultivate audit was red for a gap this task intended to close: `uv run python skills/cultivate/scripts/audit_cultivate.py .` shows that gap as `present` (or, for `CI workflows`, still `missing` by design per `seed.md` `## Deferred`).
6. No new files outside the documented layout (`skills/*`, `.agents/*`, `scripts/`, `docs/`, `.github/`, root `AGENTS.md` / `README.md` / `seed.md` / `pyproject.toml` / `uv.lock`).
...
- Edit `.agents/skills/*` directly. Change `skills/*`, then run `uv run python scripts/sync_agents_skills.py`.
- Force-push to `main`.
